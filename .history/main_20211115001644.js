...
console.log(menuToCustom);
console.log(menuToCustom[0]);
console.log(menuToCustom[1]);
console.log(typeof menuToCustom);
let array = Object.keys(menuToCustom).map(function(key) {
  return menuToCustom[key];
})
console.log(array);
menuToCustom.forEach(item => {
  item.addEventListener('click', () => {
    document.getElementById('portfolio__custom').scrollIntoView();
  })
});
menuToReadymade.forEach(item => {
  item.addEventListener('click', () => {
    document.getElementById('portfolio__readymade').scrollIntoView();
  })
});
menuToEducationProcess.forEach(item => {
  item.addEventListener('click', () => {
